...
def get_current_window():
    """Calculate the current 15-minute window timestamps."""
    now = int(time.time())
...
def get_next_window():
    """Calculate the NEXT 15-minute window (the one to trade)."""
    now = int(time.time())
    current_start = now - (now % 900)
    next_start = current_start + 900
    next_end = next_start + 900
    return next_start, next_end

def build_slug(window_start):
    """Build the Polymarket slug for a 15-min BTC market."""
